...
import { withAuth, apiError, apiSuccess, apiServerError } from '@/lib/api'
...
export const POST = withAuth('client', async (request: NextRequest) => {
  ...
  if (!file) {
    return apiError('No image provided')
  }

  const validTypes = ['image/png', 'image/jpeg', 'image/webp', 'image/gif']
  if (!validTypes.includes(file.type)) {
    return apiError('Invalid image type. Please upload PNG, JPG, WEBP, or GIF.')
  }
  ...
  if (!textContent || textContent.type !== 'text') {
    return apiError('Failed to extract menu items', 500)
  }
  ...
  try {
    ...
    return apiServerError('Failed to parse Claude response', parseError)
  }
